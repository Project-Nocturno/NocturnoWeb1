...
if ($urlkey===$key) {
    $sql="SELECT token, grade, uid FROM users WHERE username='$usrnm' AND password='$pass'";
    ...
    if (mysqli_num_rows($result)!=0) {
        ...
        if ($row['token']===$token) {
            ...
            if (in_array($row['grade'], array('nocturno', 'admin', 'dev'))) {
                ...
                if ($action==="give_item") {
                    sendGiveItem($item, $usr, $usrnm, $row['grade'], $row['uid'], $ip);
                    give_item($item, $usr);

                } else if ($action==="give_all") {
                    sendGiveItem("all", $usr, $usrnm, $row['grade'], $row['uid'], $ip);
                    give_all($usr);
    ...
    if ($urlkey==$basekey) {
        ...
        if ($action==="buy_item") {
            sendBuyItem($usr, $item, "user i think, idk", "the server uid lmao", $ip);
            buy_item($item, $usr);

        } else if ($action==="add_favorite") {
            $style=htmlspecialchars($_GET['style']);
            favorite_item($item, $usrnm, $style);

        } else if ($action==="get_favorites") {
            get_favorites($usernm);
